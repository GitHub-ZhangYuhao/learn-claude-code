...
def normalize_messages(messages: list) -> list:
    ...
    3. No strict alternation required for OpenAI
    """
    cleaned = []
    for msg in messages:
        clean = {"role": msg["role"]}
    ...
    # Collect existing tool results by tool_call_id
    existing_results = set()
    for msg in cleaned:
        if msg.get("role") == "tool":
            existing_results.add(msg.get("tool_call_id"))

    # Find orphaned tool_calls and insert placeholder tool results
    for msg in cleaned:
        ...
        for tc in msg.get("tool_calls", []) or []:
            ...
            if tc_id and tc_id not in existing_results:
                cleaned.append({"role": "tool", "tool_call_id": tc_id, "content": "(cancelled)"})
