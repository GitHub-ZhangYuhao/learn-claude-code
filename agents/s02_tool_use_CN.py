...
"""
s02_tool_use.py - 工具调度 + 消息规范化

s01中的代理循环没有改变。我们在调度映射中添加了工具，
并添加了一个normalize_messages()函数，在每次API调用前清理消息列表。
...
def normalize_messages(messages: list) -> list:
    ...
    三个任务：
    1. 剥离API不理解的内部元数据字段
    2. 确保每个tool_use都有匹配的tool_result（如果缺少则插入占位符）
    3. 合并连续的相同角色消息（API要求严格交替）
    """
    cleaned = []
    for msg in messages:
        clean = {"role": msg["role"]}
...
def agent_loop(messages: list):
    ...
    """
    while True:
        normalizeContent = normalize_messages(messages)
        response = client.messages.create(
            model=MODEL, system=SYSTEM,
            messages=normalizeContent,
