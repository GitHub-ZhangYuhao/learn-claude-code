#!/usr/bin/env python3
# Harness: 团队邮箱 —— 多个模型，通过文件协调。
"""
s15_agent_teams_CN.py - Agent 团队

持久化命名代理，基于文件的 JSONL 邮箱通信。每个队友在自己的线程中
运行独立的 agent 循环。通信通过追加式收件箱文件实现。

    Subagent (s04):  spawn -> 执行 -> 返回摘要 -> 销毁
    Teammate (s15):  spawn -> 工作 -> 空闲 -> 工作 -> ... -> 关闭
...
核心思想：队友拥有独立名称、收件箱和循环。

阅读顺序：
1. MessageBus：消息如何入队和清空。
2. TeammateManager：队友持久状态的定义。
3. _teammate_loop / TOOL_HANDLERS：每个队友如何反复进入工具循环。
...
SYSTEM = f"你是位于 {WORKDIR} 的团队负责人。创建队友并通过邮箱通信。"
...
class MessageBus:
    ...
    def send(self, sender: str, to: str, content: str,
        ...
        if msg_type not in VALID_MSG_TYPES:
            return f"Error: 无效类型 '{msg_type}'。有效值: {VALID_MSG_TYPES}"
        msg = {
            "type": msg_type,
            "from": sender,
            "content": content,
...
# -- TeammateManager：持久化命名代理，附带 config.json --
...
TOOLS = [
    ...
    {"name": "edit_file", "description": "在文件中精确替换文本。",
     "input_schema": {"type": "object", "properties": {"path": {"type": "string"}, "old_text": {"type": "string"}, "new_text": {"type": "string"}}, "required": ["path", "old_text", "new_text"]}},
    {"name": "spawn_teammate", "description": "创建持久化队友，在自己的线程中运行。",
